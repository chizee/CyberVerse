...
func (r *Router) handleTestCharacterVoice(w http.ResponseWriter, req *http.Request) {
	...
	if provider != "doubao" && provider != "qwen_omni" && provider != "grok" && provider != "gemini" && provider != "openai_realtime" {
...
func isDoubaoTTSModel(model string) bool {
	model = strings.ToLower(strings.TrimSpace(model))
	return strings.HasPrefix(model, "seed-tts-") || strings.HasPrefix(model, "seed-icl-")
}
