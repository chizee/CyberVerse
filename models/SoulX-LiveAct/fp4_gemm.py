...
_CUSTOM_OPS_AVAILABLE = hasattr(torch, "library") and hasattr(torch.library, "custom_op")
...
if _CUSTOM_OPS_AVAILABLE:
    try:

        @torch.library.custom_op(
            "cyberverse_liveact::scaled_nvfp4_quant",
            mutates_args=(),
        )
        def _scaled_nvfp4_quant(
            input: torch.Tensor,
            input_global_scale: torch.Tensor,
        ) -> tuple[torch.Tensor, torch.Tensor]:
            scaled_nvfp4_quant, _ = _load_lightx2v_kernel()
            return scaled_nvfp4_quant(input, input_global_scale)

        @_scaled_nvfp4_quant.register_fake
        def _(
            input: torch.Tensor,
            input_global_scale: torch.Tensor,
        ) -> tuple[torch.Tensor, torch.Tensor]:
            del input_global_scale
            m, n = input.shape
            rounded_m = ((m + 128 - 1) // 128) * 128
            scale_cols = ((n // 16 + 4 - 1) // 4) * 4
            return (
                torch.empty((m, n // 2), device=input.device, dtype=torch.uint8),
                torch.empty(
                    (rounded_m, scale_cols),
                    device=input.device,
        ...
        @torch.library.custom_op(
            "cyberverse_liveact::cutlass_scaled_nvfp4_mm",
            mutates_args=(),
        )
        def _cutlass_scaled_nvfp4_mm(
            mat_a: torch.Tensor,
            mat_b: torch.Tensor,
            ...
            _, cutlass_scaled_nvfp4_mm = _load_lightx2v_kernel()
        ...
        @_cutlass_scaled_nvfp4_mm.register_fake
        def _(
            mat_a: torch.Tensor,
            mat_b: torch.Tensor,
            scales_a: torch.Tensor,
            ...
            del scales_a, scales_b, alpha, bias
            return torch.empty(
...
class FP4Linear(nn.Module):
    ...
    def _maybe_requantize_weight(self, device: torch.device) -> None:
        ...
        if self.linear is not None:
            ...
            if (
                self._fp4_weight is not None
                and self._fp4_weight_scale is not None
        ...
        with torch.cuda.device(device):
            qweight, w_scale = _scaled_nvfp4_quant(w_n_k, w_global_scale)
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        if not x.is_cuda:
            ...
            if self._fp16_weight_cpu is not None:
                ...
                return torch.nn.functional.linear(
                    x,
                    self._fp16_weight_cpu.to(dtype=x.dtype),
        ...
        if in_dtype not in (torch.float16, torch.bfloat16):
            if not self.options.cast_inputs:
                ...
                raise RuntimeError(
                    "cast_inputs=False requires FP16 weights for fallback, "
        ...
        with torch.cuda.device(x_fp.device):
            q_x, s_x = _scaled_nvfp4_quant(x_2d, input_global_scale)

            alpha = 1.0 / (input_global_scale * self._w_global_scale)
            y_2d = _cutlass_scaled_nvfp4_mm(
                q_x,
                self._fp4_weight,
                s_x,
...
def materialize_fp4_gemm(model: nn.Module, device: torch.device | str | int) -> nn.Module:
    device = torch.device(device)
    for module in model.modules():
        if isinstance(module, FP4Linear):
            module.materialize_fp4_weight(device)
    return model
